...
//
 {
  for(unsigned short i = 0 ; i < probabilities.size();++i) {
    ...
    up[i] = std::inner_product(begin(rule_probs_i), end(rule_probs_i), begin(left), up[i]);
    ...
    assert(up[i] >= 0.0 - std::numeric_limits<double>::epsilon());
    assert(up[i] <= 1.0 + std::numeric_limits<double>::epsilon());
  }
}
...
void URuleTraining::split(unsigned n, unsigned randomness)
{
  ...
  for(unsigned i = 0 ; i < probabilities.size(); ++ i) {
	...
	for(unsigned annot_b = 0 ; annot_b < n ; ++annot_b) {
	  ...
	  assert(new_probabilities[new_i][range_j+annot_b] <= 1.0 + std::numeric_limits<double>::epsilon());
	  assert(new_probabilities[new_i][range_j+annot_b] >= 0.0 - std::numeric_limits<double>::epsilon());
