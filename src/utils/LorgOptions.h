 ...
 public:
  ...
  void add_parser_options()
  {
    ...
    parser_opts.add_options()
        ...
        ("input-mode,i",po::value<std::string>()->default_value("tok"),"sets the input mode: \"raw\", \"tok\", \"tag\" or \"lat\"")
        ("filter-tagging", po::value<std::string>()->default_value("none"), "none or bikel" )
  ...
  void add_nn_parser_options()
  {
    ...
    parser_opts.add_options()
        ("batch-size", po::value<unsigned>()->default_value(10U), "size of batch")
        ("iterations", po::value<unsigned>()->default_value(30U), "number of iterations")
        ...
        ("use-span-midpoints", po::value<bool>()->default_value(false), "additionally use split word to characterize spans (sloow)")
        ;
    options.add(parser_opts);
  }
